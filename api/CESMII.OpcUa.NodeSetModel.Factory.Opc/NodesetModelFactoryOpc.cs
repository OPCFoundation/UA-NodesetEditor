...
namespace CESMII.OpcUa.NodeSetModel.Factory.Opc
{
    ...
    public class NodeModelFactoryOpc : NodeModelFactoryOpc<NodeModel>
    {
        public static Task<List<NodeSetModel>> LoadNodeSetAsync(IOpcUaContext opcContext, UANodeSet nodeSet, Object customState, Dictionary<string, string> Aliases, bool doNotReimport = false, List<NodeState> importedNodes = null)
        {
            ...
            int iItem = 1;
            int count = newImportedNodes.Count;
            foreach (var node in newImportedNodes)
            {
                ///// System.Diagnostics.Debug.WriteLine($"Creating Node [{iItem+1}/{count}] in {nodeSet.Models[0].ModelUri}  for {node.DisplayName} {node.NodeId}");
                ///// System.Diagnostics.Debug.WriteLine("Create Calling #1 at Line [90]");
                var nodeModel = NodeModelFactoryOpc.Create(opcContext, node, customState, out var bAdded, doNotReimport);
            ...
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #2 at Line [105]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(ReferenceTypes.HasSubtype), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #3 at Line [107]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(ReferenceTypes.HasModellingRule), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #4 at Line [109]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(Objects.ModellingRule_Mandatory), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #5 at Line [111]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(Objects.ModellingRule_Optional), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #6 at Line [113]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(Objects.ModellingRule_ExposesItsArray), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #7 at Line [115]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(Objects.ModellingRule_MandatoryPlaceholder), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #8 at Line [117]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(Objects.ModellingRule_OptionalPlaceholder), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #9 at Line [119]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(ReferenceTypes.HasTypeDefinition), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #10 at Line [121]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(ReferenceTypes.GeneratesEvent), null, out _, doNotReimport);
            ///// System.Diagnostics.Debug.WriteLine("Create Calling #11 at Line [123]");
            ReferenceTypeModelFactoryOpc.Create(opcContext, opcContext.GetNode(ReferenceTypes.Organizes), null, out _, doNotReimport);
    ...
    public class NodeModelFactoryOpc<TNodeModel> where TNodeModel : NodeModel, new()
    {
        ...
        protected virtual void Initialize(IOpcUaContext opcContext, NodeState opcNode, int recursionDepth)
        {
            ...
            foreach (var reference in references)
            {
                ...
                {
                    bool bToDoXNodesetRef = false;

                    // TODO UANodeSet.Import should already handle inverse references: investigate why these are not processed
                    // Workaround for now:
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #12 at Line [192]");
                    AddChildToNodeModel(
                        () => NodeModelFactoryOpc<TNodeModel>.Create(opcContext, referencedNode, this._model.CustomState, out _, bToDoXNodesetRef, recursionDepth),
        ...
        private static void AddChildToNodeModel(Func<NodeModel> parentFactory, IOpcUaContext opcContext, ReferenceTypeState referenceType, List<BaseTypeState> referenceTypes, NodeState referencedNode, int recursionDepth)
        {
            ...
            if (referenceTypes.Any(n => n.NodeId == ReferenceTypeIds.HasComponent))
            {
                if (referencedNode is BaseObjectState objectState)
                {
                    ...
                    {
                        bool bCrossNamespaceReference = parent?.Namespace != uaChildObject.Namespace;
                        ///// System.Diagnostics.Debug.WriteLine("Create Calling #13 at Line [217]");
                        var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bCrossNamespaceReference, recursionDepth) as ReferenceTypeModel;
                        // if (parent?.Namespace != uaChildObject.Namespace)
                        if (bCrossNamespaceReference) // What about checking whether referenceTypeModel is not null??
                        {
                ...
                else if (referencedNode is BaseDataVariableState variableState)
                {
                    ...
                    bool bToDoXNodesetRef = false;
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #14 at Line [252]");
                    var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
                    if (referenceTypes[0].NodeId != ReferenceTypeIds.HasComponent)
                    {
                        // Preserve the more specific reference type as well
                ...
                else if(referencedNode is PropertyState propertyState)
                {
                    ...
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #15 at Line [276]");
                    var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
                    if (referenceTypes[0].NodeId != ReferenceTypeIds.HasComponent)
                    {
                        // Preserve the more specific reference type as well
            ...
            else if (referenceTypes.Any(n => n.NodeId == ReferenceTypeIds.HasProperty))
            {
                ...
                if (referencedNode is PropertyState propertyState)
                {
                    ...
                    bool bToDoXNodesetRef = false;
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #16 at Line [332]");
                    var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
                ...
                else if (referencedNode is BaseDataVariableState variableState)
                {
                    ...
                    bool bToDoXNodesetRef = false;
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #17 at Line [349]");
                    var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
                    if (referenceTypes[0].NodeId != ReferenceTypeIds.HasProperty)
                    {
                        // Preserve the more specific reference type as well
            ...
            else if (referenceTypes.Any(n => n.NodeId == ReferenceTypeIds.HasInterface))
            {
                ...
                if (referencedNode is BaseObjectTypeState interfaceTypeState)
                {
                    ...
                    if (uaInterface != null)
                    {
                        ...
                        bool bToDoXNodesetRef = false;
                        ///// System.Diagnostics.Debug.WriteLine("Create Calling #18 at Line [381]");
                        var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
            ...
            else if (referenceTypes.Any(n => n.NodeId == ReferenceTypeIds.GeneratesEvent))
            {
                ...
                if (referencedNode is BaseObjectTypeState eventTypeState)
                {
                    ...
                    if (uaEvent != null)
                    {
                        ...
                        bool bToDoXNodesetRef = false;
                        ///// System.Diagnostics.Debug.WriteLine("Create Calling #19 at Line [434]");
                        var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef, recursionDepth) as ReferenceTypeModel;
            ...
            else
            {
                ...
                bool bToDoXNodesetRef1 = false;
                ///// System.Diagnostics.Debug.WriteLine("Create Calling #20 at Line [456]");
                var referencedModel = Create(opcContext, referencedNode, parent?.CustomState, out _, bToDoXNodesetRef1, recursionDepth);
                if (referencedModel != null)
                {
                    bool bToDoXNodesetRef2 = false;

                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #21 at Line [462]");
                    var referenceTypeModel = ReferenceTypeModelFactoryOpc.Create(opcContext, referenceType, null, out _, bToDoXNodesetRef2, recursionDepth) as ReferenceTypeModel;
        ...
        static bool ProcessEUInfoAndRangesWithoutParent(IOpcUaContext opcContext, NodeState potentialEUNode, object customState)
        {
                ...
                || potentialEUNode.BrowseName?.Name == BrowseNames.EURange || potentialEUNode.BrowseName?.Name == BrowseNames.InstrumentRange)
            {
                foreach (var referenceToNode in opcContext.GetHierarchyReferences(potentialEUNode).Where(r => r.IsInverse))
                {
                    ...
                    bool bToDoXNodesetRef = false;
                    ///// System.Diagnostics.Debug.WriteLine("Create Calling #22 at Line [525]");
                    var referencingNode = Create(opcContext, referencingNodeState, customState, out _, bToDoXNodesetRef);
        ...
        public static NodeModel Create(IOpcUaContext opcContext, NodeState node, object customState, out bool added, bool bCrossNamespaceReference, int recursionDepth = int.MaxValue)
        {
            if (bCrossNamespaceReference)
            {
                int i = 123;
            }
            NodeModel nodeModel;
        ...
        where TBaseTypeModelFactoryOpc : NodeModelFactoryOpc<TBaseTypeModel>, new()
    {
        protected override void Initialize(IOpcUaContext opcContext, NodeState opcNode, int recursionDepth)
        {
            ...
            {
                bool bToDoXNodesetRef = false;
                ///// System.Diagnostics.Debug.WriteLine("Create Calling #23 at Line [884]");
                var typeDefModel = NodeModelFactoryOpc.Create(opcContext, variableTypeDefinition, _model.CustomState, out _, bToDoXNodesetRef, recursionDepth - 1); // Create<TBaseTypeModelFactoryOpc, TBaseTypeModel>(opcContext, variableTypeDefinition, null);
            ...
            {
                bool bToDoXNodesetRef = false;
                ///// System.Diagnostics.Debug.WriteLine("Create Calling #24 at Line [906]");
                var instanceParent = NodeModelFactoryOpc.Create(opcContext, uaInstance.Parent, null, out _, bToDoXNodesetRef, recursionDepth - 1);
    ...
    public class BaseTypeModelFactoryOpc<TBaseTypeModel> : NodeModelFactoryOpc<TBaseTypeModel> where TBaseTypeModel : BaseTypeModel, new()
    {
        ...
        private static Dictionary<string, object> DictNamespaceToDbContext = new Dictionary<string, object>();

        protected void AddToDbContextDictionary(string strNamespace, object o)
        {
            if (!DictNamespaceToDbContext.ContainsKey(strNamespace))
            {
                DictNamespaceToDbContext.Add(strNamespace, o);
            }
        }

        protected object? GetDbContextFromNamespace(string strNamespace)
        {
            if (DictNamespaceToDbContext.ContainsKey(strNamespace))
                return DictNamespaceToDbContext[strNamespace];
            else
                return null;
        }
            ...
            if (uaType.SuperTypeId != null)
            {
                ...
                if (superTypeModel == null)
                {
                    ...
                    if (superTypeState != null)
                    {
                        ...
                        ///// System.Diagnostics.Debug.WriteLine("Create Calling #25 at Line [982]");
                        superTypeModel = NodeModelFactoryOpc.Create(opcContext, superTypeState, this._model.CustomState, out _, bSameNamespace, 2) as BaseTypeModel;
                        if (superTypeModel == null)
                        {
                ...
                {
                    opcContext.Logger.LogWarning($"Error in call to RemoveInheritedAttributes");
                }
            }
            else
            {

...
namespace CESMII.OpcUa.NodeSetModel.Export.Opc
{
    ...
    public class NodeModelExportOpc<T> where T : NodeModel, new()
    {
        ...
        {
            iCallCount++;
            System.Diagnostics.Debug.WriteLine($"GetUANode called. _model.NodeId={_model.NodeId}");
            ...
            {
                System.Diagnostics.Debug.WriteLine($"GetUANode. In OtherReferencedNodes loop.");
                // Break apart to aid in debugging.
                // Before:
                ...
                // My debug helper/
                var iFirstNamespace = nodeRef.Node.Namespace;
                var iFirstIndex = context.NamespaceUris.GetIndexOrAppend(iFirstNamespace);

                var myNodeId = nodeRef.ReferenceType?.NodeId;
                var myNamespace = NodeModelUtils.GetNamespaceFromNodeId(myNodeId);
                // End debug helper

                // Patch anything without a namespace to use the core namespace.
                if (string.IsNullOrEmpty(myNamespace))
                {
                    if (myNodeId != null && myNodeId.Contains("nsu=;"))
                    {
                        myNodeId = myNodeId.Replace("nsu=;", "nsu=http://opcfoundation.org/UA/;");
                        nodeRef.ReferenceType.NodeId = myNodeId;
                        myNamespace = "http://opcfoundation.org/UA/";
                        System.Diagnostics.Debug.WriteLine($"GetUANode. In OtherReferencedNodes loop. Switcharoo on myNodeId={myNodeId}");
                    }
                }
                context.NamespaceUris.GetIndexOrAppend(myNamespace);
            ...
            foreach (var inverseNodeRef in this._model.OtherReferencingNodes)
            {
                ...
                // My debug helper
                var iFirstNamespace = inverseNodeRef.Node.Namespace;
                var iFirstIndex = context.NamespaceUris.GetIndexOrAppend(iFirstNamespace);

                var theNodeId = inverseNodeRef.ReferenceType?.NodeId;
                var theSecondNamespace = NodeModelUtils.GetNamespaceFromNodeId(theNodeId);
                // Patch anything without a namespace to use the core namespace.
                if (string.IsNullOrEmpty(theSecondNamespace))
                {
                    if (theNodeId != null && theNodeId.Contains("nsu=;"))
                    {
                        theNodeId = theNodeId.Replace("nsu=;", "nsu=http://opcfoundation.org/UA/;");
                        inverseNodeRef.ReferenceType.NodeId = theNodeId;
                        theSecondNamespace = "http://opcfoundation.org/UA/";
                        System.Diagnostics.Debug.WriteLine($"GetUANode. ******************************* In OtherReferencedNodes loop. Switcharoo on theNodeId={theNodeId}");
                    }
                }


                var AnotherIndex = context.NamespaceUris.GetIndexOrAppend(theSecondNamespace);
                // End debug helper
